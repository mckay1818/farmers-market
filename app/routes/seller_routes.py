...
def update_one_seller(store_name):
    ...
    try:
        current_user.store_name = request_body["store_name"]
        current_user.store_descriptions = request_body["store_description"]
    ...
    db.session.commit()
    return make_response(jsonify(f"Seller {current_user.first_name} {current_user.last_name}, owner of {current_user.store_name} successfully updated."), 200)
...
def delete_one_seller(store_name):
    ...
    db.session.commit()
    return make_response(jsonify(f"Seller {current_user.first_name} {current_user.last_name}, owner of {current_user.store_name} successfully deleted."), 200)
